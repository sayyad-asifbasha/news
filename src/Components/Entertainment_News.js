...
// let url="https://newsapi.org/v2/everything?q=entertainment&&sortBy=popularityapiKey=3250e7d8113e475bbf5f9e6f432dda5c";

// let url="https://newsapi.org/v2/everything?q=entertainment&&sortBy=popularityapiKey=dc3e6eded1584bd7bbb7dedba5ec17b5";

let url="https://newsapi.org/v2/everything?q=entertainment&sortBy=popularity&apiKey=03ded8b29e214785ac1c6e4b0876b395";


export default function Entertainment_News() {
  ...
  const allocateData = async () => {
    ...
    if (!localStorage.getItem('Entertainment')) {
      ...
      localStorage.setItem('Entertainment', JSON.stringify(result));
      setData(JSON.parse(localStorage.getItem('Entertainment')));
      console.log("hiited")
    }
    else {
      setData(JSON.parse(localStorage.getItem('Entertainment')));
