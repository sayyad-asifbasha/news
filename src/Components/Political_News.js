...
// let url="https://newsapi.org/v2/everything?q=politics and india&apiKey=3250e7d8113e475bbf5f9e6f432dda5c";
// let url="https://newsapi.org/v2/everything?q=politics&apiKey=dc3e6eded1584bd7bbb7dedba5ec17b5";
let url="https://newsapi.org/v2/everything?q=politics and india&apiKey=03ded8b29e214785ac1c6e4b0876b395";

export default function Political_News() {
  const [nextCount,setNextCount]=useState(6);
  const [data, setData] = useState({});
  const allocateData = async () => {

    if (!localStorage.getItem('Political')) {
      const response = await axios.get(url);
      let result = response.data;
      localStorage.setItem('Political', JSON.stringify(result));
      setData(JSON.parse(localStorage.getItem('Political')));
      console.log("hiited")
    }
    else {
      setData(JSON.parse(localStorage.getItem('Political')));
